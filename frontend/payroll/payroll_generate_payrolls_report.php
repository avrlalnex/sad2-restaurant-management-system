  ...
  if($_SERVER['REQUEST_METHOD']=="POST"){
    if($_POST['submit'] == "Generate"){
      ...
      if($samedateresult == true){
        ...
        $i=0;
        while($i < $_POST['max_emp']){
          if($_POST['emp_generate_'.$i]==1){
            $result = $inv_tbl->generate_payroll($_POST['rep-floorDate'],$_POST['rep-ceilingDate'],$_POST['emp_id_'.$i]);
          }
          $i++;
        }
        header("Location: payroll_add_report_generated.php");
        exit();
      }
    }
  }
...
<form method="POST">
  ...
  <a href="payroll.php" class="return-icon"><ion-icon name="arrow-back-outline"></ion-icon></a> 
<div class="sales-entry">
  <h2>Generate payrolls</h2>
  ...
  <div class="input-container">
    <input type="submit" class="add-button" name="submit" value="Generate">
  </div>
</div>
</div>
</div>
<div class="inventory-table">
    ...
    <tbody>
      <?php $inv_tbl->show_employees_generate();?> 
    </tbody>
  </table>
</div>
</form>
