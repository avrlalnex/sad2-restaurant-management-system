<?php
  ...
  if($_SERVER['REQUEST_METHOD']=="POST"){
    if($_POST['submit'] == "Archive"){
      $result = $inv_tbl->archive_receipt($_SESSION['receiptView_id']);
      header("Location: sales.php");
      exit();
    }else if($_POST['submit'] == "Edit"){
      header("Location: sales_edit_receipt.php");
      exit();
    }
  }
?>
...
<section class="home-section">
  <div class="text">Sales - View Receipt</div>
    <div class="add-item-container">
      <div class="employee-info">
      <div class="sales-entry">
  <a href="sales.php" class="return-icon"><ion-icon name="arrow-back-outline"></ion-icon></a></br>
  <h2>View Sales</h2>
  <input type="submit" class="add-button" name="submit" value="Edit">
  <input type="submit" class="add-button" name="submit" value="Archive" onclick="return confirm('Are you sure you want to Archive?')">
  <div class="input-container">
    <label for="receiptNumber">Receipt Number:</label>
    <input type="text" id="receiptNumber" name="receiptNumber" value="<?php echo $result=$inv_tbl->get_sales_receipt_detail($_SESSION['receiptView_id'],"receipt number");?>" readonly>
  </div>
  <div class="input-container">
    <label for="receiptTable">Table Number:</label>
    <input type="text" id="receiptTable" name="receiptTable" value="<?php echo $result=$inv_tbl->get_sales_receipt_detail($_SESSION['receiptView_id'],"table number");?>" readonly oninput="check_num(this.value,this.id)">
  </div>
  <div class="input-container">
    <label for="custName">Customer:</label>
      <input type="text" id="custName" name="custName" value="<?php echo $result=$inv_tbl->get_sales_receipt_detail($_SESSION['receiptView_id'],"customer name");?>" readonly>
  ...
  <div class="input-container">
    ...
    <input type="text" id="custName" name="custName" value="<?php echo $result=$inv_tbl->get_sales_receipt_detail($_SESSION['receiptView_id'],"product name");?>" readonly>
    <select id="xpayRate" name="xpayRate" onchange="receipt_selectRate(this.value);receipt_calculateTotal()" hidden>
      <?php $inv_tbl->get_sales_product_default();?> 
  ...
  <div class="input-container">
    ...
    <input type="number" id="numPax" name="numPax" value="<?php echo $result=$inv_tbl->get_sales_receipt_detail($_SESSION['receiptView_id'],"pax");?>" oninput="check_paxValid(this.value);receipt_calculateTotal()" readonly>
  </div>
  <div class="input-container">
    <label for="discType">Discount Type:</label>
    <input type="text" id="custName" name="custName" value="<?php echo $result=$inv_tbl->get_sales_receipt_detail($_SESSION['receiptView_id'],"discount type");?>" readonly>
    <label for="seniorPax" id="seniorPaxLabel">Senior Pax</label>
    <input type="number" id="seniorPax" name="seniorPax" value="<?php echo $result=$inv_tbl->get_sales_receipt_detail($_SESSION['receiptView_id'],"senior pax");?>" readonly oninput="check_seniorValid(this.value);receipt_calculateTotal()">
  </div>
  <div class="input-container">
    <label for="totAmount">Total Amount:</label>
    <!-- <small><small><small>Pax*Price per pax - Senior Pax*(VAT Exempt Price*20%)</small></small></small><br>
    <small id="totAmount_calc">1*0 - 0*0</small> -->
    <input type="text" id="totAmount" name="totAmount" value="<?php echo $result=$inv_tbl->get_sales_receipt_detail($_SESSION['receiptView_id'],"total amount");?>" oninput="check_num(this.value,this.id)" readonly>
  </div>
  <div class="input-container">
    <label for="receiptDate">Date:</label>
    <input type="date" id="receiptDate" name="receiptDate" value="<?php echo $result=$inv_tbl->get_sales_receipt_detail($_SESSION['receiptView_id'],"receipt date");?>" readonly>
  </div>
</div>
</section>
<!--<div class="add-item-container">
    <div class="input-container">
      ...
      <select id="stock-id-goods" name="stock-id-goods">
        ...
        <input type="hidden" id="invoice-numofitems" name="invoice-numofitems" value=0 readonly>
    </div>
  </div> -->
<div class="inventory-table">
    <table id="invoice_items" class="item-table">
      ...
      <tbody>
        <?php $inv_tbl->show_sales_receipt_addons($_SESSION['receiptView_id'])?>
      </tbody>
    </table>
  </div>
</form>
