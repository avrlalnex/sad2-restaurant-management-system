<?php
  ...
  if($_SERVER['REQUEST_METHOD']=="POST"){
    if($_POST['submit'] == "Add"){
      $result = $inv_tbl->add_receipt($_POST['receiptNumber'],$_POST['custName'],$_POST['numPax'],$_POST['totAmount'],$_POST['discType'],$_POST['receiptDate'],$_POST['seniorPax'],$_POST['payPax'],$_POST['receiptTable']);
      $_SESSION['invoice-id'] = $inv_tbl->get_last_receipt();
      $result = $inv_tbl->add_receipt_product($_SESSION['invoice-id'],$_POST['payRate'],$_POST['numPax'],($_POST['payPax']*$_POST['numPax']));
      $i=0;
      while($i < $_POST['invoice-numofitems']) {
        $result = $inv_tbl->add_receipt_product($_SESSION['invoice-id'],$_POST['invoiceItem_id_'.$i],$_POST['invoiceItem_quant_'.$i],$_POST['invoiceItem_price_'.$i]);
        $i++;
      }
    }
  }
...
<section class="home-section">
  ...
  <a href="sales.php" class="return-icon"><ion-icon name="arrow-back-outline"></ion-icon></a> 
  <h2>Enter Sales</h2>
  <div class="input-container">
    <label for="receiptNumber">Receipt Number:</label>
    <input type="text" id="receiptNumber" name="receiptNumber" value="<?php echo $result=$inv_tbl->get_next_receipt_no();?>">
  </div>
  <div class="input-container">
    <label for="receiptTable">Table Number:</label>
    <input type="text" id="receiptTable" name="receiptTable" value=1 oninput="check_num(this.value,this.id)">
  </div>
  <div class="input-container">
  ...
  <div class="input-container">
    ...
    <input type="number" id="payPax" name="payPax" value="<?php echo $result=$inv_tbl->get_sales_product_default_price();?>" oninput="check_num(this.value,this.id);receipt_calculateTotal()" readonly>
    <?php $inv_tbl->get_sales_product_unli_price();$inv_tbl->get_sales_product_addon_price()?>
  </div>
  <div class="input-container">
    <label for="numPax">Number of Pax:</label>
    <input type="number" id="numPax" name="numPax" value="1" oninput="check_paxValid(this.value);receipt_calculateTotal()">
  </div>
  <div class="input-container">
    ...
    <select id="discType" name="discType" onchange="receipt_selectSenior();receipt_calculateTotal()">
        <option value="Not Discounted" selected>Not Discounted</option> 
        <option value="Senior">Senior</option>
        <option value="Other">Other</option>
    </select>
    <label for="seniorPax" id="seniorPaxLabel"></label>
    <input type="number" id="seniorPax" name="seniorPax" value="0" hidden oninput="check_seniorValid(this.value);receipt_calculateTotal()">
  </div>
  <div class="input-container">
    ...
    <input type="text" id="totAmount" name="totAmount" value="<?php echo $result=$inv_tbl->get_sales_product_default_price();?>" oninput="check_num(this.value,this.id)">
  </div>
  <div class="input-container">
    <label for="receiptDate">Date:</label>
    <input type="date" id="receiptDate" name="receiptDate" value="<?php $currentDate = date('Y-m-d'); echo $currentDate?>">
  </div>
  <div class="input-container">
    <input type="submit" class="add-button" name="submit" value="Add">
  </div>
