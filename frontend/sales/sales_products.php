<?php
  ...
  if($_SERVER['REQUEST_METHOD']=="POST"){
    if($_POST['submit'] == "Save Changes"){
      $i=0;
      while($i < $_POST['0_max_num']) {
        $result = $inv_tbl->edit_product($_POST['0_product_id_'.$i],$_POST['0_product_name_'.$i],$_POST['0_product_price_'.$i]);
        $i++;
      }
      $i=0;
      while($i < $_POST['1_max_num']) {
        $result = $inv_tbl->edit_product($_POST['1_product_id_'.$i],$_POST['1_product_name_'.$i],$_POST['1_product_price_'.$i]);
        $i++;
      }
      $result = $inv_tbl->edit_product_default($_POST['product_default']);
      header("Location: sales.php");
      exit();
    }else if($_POST['submit'] == "Archive"){
      $result = $inv_tbl->archive_product($_POST['product_id_archive']);
    }
  }
?>
<!DOCTYPE html>
...
<section class="home-section">
  ...
  <div class="inventory-table">
    <a href="sales.php"><input type="button" value="Return" class="add-button" onclick="return confirm('Unsaved Changes will be Discarded')"></a>
    <input type="submit" name="submit" value="Save Changes" class="add-button" onclick="return confirm('Save Changes?')">
    <a href="sales_add_product.php"><input type="button" value="Add Product" class="add-button" onclick="return confirm('Unsaved Changes will be Discarded')"></a>
    <a href="sales_products_archive.php"><input type="button" value="Products Archive" class="add-button"></a><br>
    <table class="item-table" style="width:45%;float:left">
    <thead>
      <tr>
        <th colspan="3">Unlimited, Default:
          <select id="product_default" name="product_default">
    ...
    <tbody>
      <?php $inv_tbl->show_sales_products(0);?> 
    </tbody>
  </table>
  <table class="item-table" style="width:45%;float:left;margin-left:5%">
    ...
    <tbody>
      <?php $inv_tbl->show_sales_products(1);?> 
    </tbody>
  </table>
  </form>
