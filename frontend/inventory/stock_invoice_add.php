<?php
  ...
  if($_SERVER['REQUEST_METHOD']=="POST"){
    if($_POST['submit'] == "Add Receiving Report"){
      ...
      while($i < $_POST['orderslip-numofitems']) {
        $result = $inv_tbl->add_stock_in($_SESSION['add-invoice-id'],$_POST['orderitem-id_'.$i],$_POST['orderitem-quant_'.$i],$_POST['invoice-expiry_'.$i]);
        $i++;
      }
      header("Location: stock_invoice_page.php");
      exit();
    }else if($_POST['submit'] == "Return"){
      header("Location: stock_invoice_purchase_order_select.php");
      exit();
    }
  }
?>
...
<section class="home-section">
  <div class="text">Receiving Report: Add</div>
  ...
  <div class="add-item-container" style="float:left;width:30%">
    <div class="add-item">
      <input type="submit" name="submit" value="Add Receiving Report" class="add-button" onclick="return confirm('Add Receiving Report?')">
