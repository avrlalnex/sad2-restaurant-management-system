<?php
  ...
  if($_SERVER['REQUEST_METHOD']=="POST"){
    if($_POST['submit'] == "Save Changes"){
      $result = $inv_tbl->edit_invoice($_SESSION['invoiceView_id'],$_POST['orderslip-num'],$_POST['delivery-date']);
      $i=0;
      while($i < $_POST['orderslip-numofitems']) {
        $result = $inv_tbl->edit_stock_in($_POST['invoiceitem-id_'.$i],$_POST['invoiceitem-quant_'.$i],$_POST['activeitem-quant_'.$i],$_POST['invoice-expiry_'.$i]);
        $i++;
      }
      header("Location: stock_invoice_view.php");
      exit();
...
<section class="home-section">
  <div class="text">Receiving Report: Edit</div>
  ...
  <div class="add-item-container" style="float:left;width:30%">
    <div class="add-item">
      ...
      <input type="text" id="orderslip-num" name="orderslip-num" value="<?php echo $info = $inv_tbl->get_invoice_info($_SESSION['invoiceView_id'],'invoice_no')?>"><br>
      </div>
      <div class="input-container">
      <label for="delivery-date">Delivery Date:</label>
      <input type="date" id="delivery-date"  name="delivery-date" value="<?php echo $info = $inv_tbl->get_invoice_info($_SESSION['invoiceView_id'],'invoice_date')?>"><br>
      </div>
    </div>
  </div>
  ...
  <table class="item-table">
    <table id="invoice_items" class="item-table">
      ...
      <tbody>
      <?php $inv_tbl->show_invoice_items_edit($_SESSION['invoiceView_id'])?>
      </tbody>
    </table>
  </div>
  </form>
